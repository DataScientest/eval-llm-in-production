...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    for error in exc.errors():
        ...
        if error_type == "string_pattern_mismatch" and "model" in error_loc:
            if (
                "../" in str(error_input)
                or "passwd" in str(error_input)
                or "\\" in str(error_input)
            ):
                security_relevant = True
                incident_type = "model_path_traversal_attempt"
        elif error_type == "greater_than_equal" and any(
            field in error_loc for field in ["max_tokens", "temperature"]
        ):
        ...
        if security_relevant:
            ...
            try:
                ...
                logger.warning(f"Could not trace validation error: {trace_error}")
